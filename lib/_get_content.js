...
export async function getPagesBooksAndChapters(nocache = false) {
    ...
    for (const page of pages.data) {
        let path
        let book = bookById[page.book_id]

        if (!book) {
            book = { slug: page.book_slug }
        }
        ...
        if (page.slug == 'index' && page.book_slug == 'default') {
            ...
            path = page.slug
        } else if (page.slug == 'index' && book.name) {
        ...
        if (page.chapter_id != 0) {
            if (!pagesByChapterId[page.chapter_id]) pagesByChapterId[page.chapter_id] = new Set()
